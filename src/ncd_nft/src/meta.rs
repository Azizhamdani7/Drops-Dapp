...
const CALLING_ACCOUNT : &str = "drops_contract.testnet";


pub trait Meta{
    fn metadata_insert(&mut self, token_type :String, metadata: TokenMetadata);
    fn nft_custom_mint(&mut self, quantity: u128, receiver_id: AccountId, token_type: TokenType); 
}
#[near_bindgen]
...
fn nft_custom_mint(&mut self, quantity:u128, receiver_id: AccountId, token_type: TokenType){
    let mut x=0;
    while x < quantity {
        let metadata = self.token_metadata.get(&token_type).unwrap();
